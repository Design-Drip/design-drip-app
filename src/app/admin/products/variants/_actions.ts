  ...
  try {
    ...
    return {
      ...
      images: color.images?.map((img: any) => ({
        ...
        editable_area: img.width_editable_zone || img.height_editable_zone ? {
...
export async function addProductColor(formData: FormData) {
  ...
  try {
    ...
    if (existingColor) {
      return { success: false, message: "Color already exists for this product" };
    }
    ...
    await Promise.all(FIXED_SIZES.map(size => 
...
export async function deleteProductColor(formData: FormData) {
  ...
  try {
    ...
    return { success: false, message: error.message || "Failed to delete color" };
  }
}
...
export async function addProductSize(formData: FormData) {
  ...
  try {
    ...
    const additional_price = parseFloat(formData.get("additional_price") as string) || 0;
...
export async function deleteProductSize(formData: FormData) {
  ...
  try {
    ...
    return { success: false, message: error.message || "Failed to delete size" };
  }
}
...
export async function updateSizePrice(formData: FormData) {
  ...
  try {
    ...
    return { success: false, message: error.message || "Failed to update size price" };
  }
}
