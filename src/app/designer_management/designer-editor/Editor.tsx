...
interface DesignerEditorProps {
  ...
  quoteId?: string; // Optional quote ID for designs from assigned quotes
}
...
export const DesignerEditor = ({
  ...
  const [selectedImage, setSelectedImage] = useState<any>(images[0] || {
    width_editable_zone: 400,
    height_editable_zone: 400,
    url: ""
  });
  ...
  const [designName, setDesignName] = useState<string>(
    designDetail?.name || "Designer Design"
  );
  ...
  const { init, editor } = useEditor({
    ...
    defaultWidth: selectedImage.width_editable_zone || 400,
    defaultHeight: selectedImage.height_editable_zone || 400,
    clearSelectionCallback: onClearSelection,
  });

  // Update selectedImage when images change
  useEffect(() => {
    if (images.length > 0 && (!selectedImage || !selectedImage.url)) {
      setSelectedImage(images[0]);
    }
  }, [images, selectedImage]);

  // Set hasUnsavedChanges to true when component mounts to ensure Save button is visible
  useEffect(() => {
  ...
  useEffect(() => {
    if (designDetail && editor?.canvas && !didLoadDesignDetail.current) {
      ...
      try {
        ...
        // Set the design name
        setDesignName(designDetail.name || "Designer Design");
  ...
  console.log("Rendering DesignerEditor return statement");
  return (
        ...
        <div className="h-full flex flex-col w-full">
          ...
          <main className="bg-muted overflow-auto flex justify-between items-center w-full flex-1 p-4 ">
              ...
              >
                <div
                  style={{
                    ...
                    top: `${selectedImage.y_editable_zone || 0}px`,
                    left: `${(selectedImage.x_editable_zone || 0) + 20}px`,
                    width: selectedImage.width_editable_zone || 400,
                    height: selectedImage.height_editable_zone || 400,
                  }}
                  ref={containerRef}
                >
                  <canvas
            ...
            <div className="flex-1 h-full flex flex-col p-4 items-center">
              <h3>Locations ({images.length} images)</h3>
              {images.length > 0 ? (
